  ...
  return (
    <div>
      ...
      <div className="mt-3 flex flex-col gap-2.5">
        {visibles.map((ficha) => {
          const abiertaActual = abierta === ficha.id;
          return (
            <Card key={ficha.id} className="p-0">
              <button
                type="button"
                onClick={() => setAbierta(abiertaActual ? null : ficha.id)}
              ...
              >
                ...
                <IconChevron
                  className={`mt-1 h-4 w-4 shrink-0 text-ink-muted transition-transform ${
                    abiertaActual ? "rotate-90" : ""
                  }`}
                />
              </button>

              {abiertaActual && (
                <div className="flex flex-col gap-3 border-t border-hairline px-4 py-4 text-[13px] dark:border-hairline-dark">
                  {ficha.resumen && (
                    <div>
                      <p className="mb-1 text-[11px] uppercase tracking-wide text-ink-muted">Resumen</p>
                      <p className="text-ink-secondary dark:text-ink-dark-secondary">{ficha.resumen}</p>
                    </div>
                  )}
                  {ficha.temas?.length > 0 && (
                    <div>
                      <p className="mb-1 text-[11px] uppercase tracking-wide text-ink-muted">Temas</p>
                      <div className="flex flex-wrap gap-1.5">
                        {ficha.temas.map((tema) => (
                          <span
                            key={tema}
                            className="rounded-full bg-hairline px-2 py-0.5 text-[12px] dark:bg-hairline-dark"
                          >
                            {tema}
                          </span>
                        ))}
                  ...
                  {ficha.terminos?.length > 0 && (
                    <div>
                      <p className="mb-1 text-[11px] uppercase tracking-wide text-ink-muted">Términos</p>
                  ...
                  {ficha.preguntas?.length > 0 && (
                    <div>
                      <p className="mb-1 text-[11px] uppercase tracking-wide text-ink-muted">Preguntas</p>
                      <ul className="list-disc space-y-0.5 pl-4 text-ink-secondary dark:text-ink-dark-secondary">
                  ...
                  {ficha.brightspace && (
                    <div>
                      <p className="mb-1 text-[11px] uppercase tracking-wide text-ink-muted">Brightspace</p>
